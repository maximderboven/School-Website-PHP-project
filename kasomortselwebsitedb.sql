...
--
-- Host: 127.0.0.1:3306
-- Gegenereerd op: 07 nov 2019 om 21:34
-- Serverversie: 5.7.23
-- PHP-versie: 7.2.10
...
--
-- Stand-in structuur voor view `vakkenoverzicht`
-- (Zie onder voor de actuele view)
--
DROP VIEW IF EXISTS `vakkenoverzicht`;
CREATE TABLE IF NOT EXISTS `vakkenoverzicht` (
`vormingsID` int(11)
,`vormingnaam` text
,`richtingID` int(11)
,`jaarID` int(11)
...
--
-- Structuur voor de view `vakkenoverzicht`
--
DROP TABLE IF EXISTS `vakkenoverzicht`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vakkenoverzicht`  AS  select `o1`.`ID` AS `vormingsID`,`o1`.`Vormingnaam` AS `vormingnaam`,`r1`.`ID` AS `richtingID`,`j1`.`ID` AS `jaarID`,`u1`.`vakkenID` AS `vakkenID`,`j1`.`jaarnaam` AS `jaarnaam`,`r1`.`richtingnaam` AS `richtingnaam`,`v1`.`vaknaam` AS `vaknaam`,`u1`.`uren` AS `uren` from ((((`uren` `u1` join `jarengraad` `j1` on((`u1`.`jarengraadID` = `j1`.`ID`))) join `vakken` `v1` on((`u1`.`vakkenID` = `v1`.`ID`))) join `richtingen` `r1` on((`u1`.`richtingenID` = `r1`.`ID`))) join `vormingen` `o1` on((`o1`.`ID` = `v1`.`IDvormingen`))) ;
COMMIT;
